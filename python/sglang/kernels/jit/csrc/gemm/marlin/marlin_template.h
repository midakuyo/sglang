...
#define STATIC_ASSERT_SCALAR_TYPE_VALID(scalar_t)               \
  static_assert(std::is_same<scalar_t, fp16_t>::value ||          \
                    std::is_same<scalar_t, bf16_t>::value, \
...
template <
    const host::ScalarTypeId a_type_id,
    const host::ScalarTypeId b_type_id,
    const host::ScalarTypeId c_type_id,
...
template <int count, host::ScalarTypeId type_id>
...
template <host::ScalarTypeId type_id>
...
__global__ void Marlin(
  ...
  if constexpr (a_type_id == host::kFE4M3fn.id()) return;
  #endif

  // sm_80 only: fp16 accumulation is a Turing-only optimisation.
  ...
  static constexpr auto a_type = host::ScalarType::from_id(a_type_id);
  static constexpr auto b_type = host::ScalarType::from_id(b_type_id);
  static constexpr auto c_type = host::ScalarType::from_id(c_type_id);
  static constexpr auto s_type = host::ScalarType::from_id(s_type_id);
  if constexpr (b_type == host::kFE2M1f) {
    static_assert(s_type == host::kFE4M3fn && group_blocks == 1 ||
                  s_type == host::kFE8M0fnu && group_blocks == 2);
  } else if constexpr (s_type == host::kFE8M0fnu) {
    // MXFP8: FP8 weights with e8m0 microscaling block scales
    static_assert(b_type == host::kFE4M3fn && group_blocks == 2);
  ...
  constexpr bool has_zp = b_type == host::kU4 || b_type == host::kU8;
  constexpr bool is_int_type = b_type == host::kU4 || b_type == host::kU8 ||
                               b_type == host::kS4 || b_type == host::kS8 ||
                               b_type == host::kU4B8 || b_type == host::kU8B128;
  // see comments of dequant.h for more details
  constexpr bool dequant_skip_flop =
      is_a_8bit || (b_type == host::kFE4M3fn && !(s_type == host::kFE8M0fnu)) ||
      b_type == host::kFE2M1f && s_type == host::kFE4M3fn ||
      has_zp && !is_zp_float && !std::is_same<scalar_t, bf16_t>::value ||
      has_zp && !is_zp_float && !(b_type == host::kU8);

  float global_scale_f32 = 1.0f;

  if constexpr (b_type == host::kFE2M1f && s_type == host::kFE4M3fn) {
  ...
  #pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (b_type_id == host::kFE2M1f.id()) {
  ...
  #pragma unroll
            ...
            float2 s_vals[2];
            if constexpr (s_type_id != host::kFE8M0fnu.id()) {
