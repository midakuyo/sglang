...
#pragma once
// Ported from vLLM v0.9.2 csrc/quantization/marlin/qqq/marlin_qqq_gemm_kernel.cu
// (QQQ: W4A8 Marlin, HandH1998) to the SGLang JIT (tvm_ffi) build. Device code
// is unchanged; base.h/mem.h helpers are inlined and the torch host wrapper is
// replaced by a TensorView entry point that takes the C/D scratch as inputs.
...
#define TORCH_CHECK(cond, ...)                                              \
  do {                                                                      \
    if (!(cond)) throw std::runtime_error(std::string("marlin_qqq: ") + #cond); \
  } while (0)
...
// ---- inlined from vLLM csrc/quantization/marlin/dense/common/base.h
/*
 * Modified by HandH1998
...
// ---- inlined from vLLM csrc/quantization/marlin/dense/common/mem.h
/*
 * Modified by HandH1998
 * Modified by Neural Magic
 * Copyright (C) Marlin.2024 Elias Frantar
...
// Entry point exported to Python. All buffers are caller-allocated:
//   a          int8   [M, K]              (per-token quantized activations)
//   b_q_weight int32  [K/16, N*16/8]      (QQQ-packed int4 weights)
//   s_tok      fp32   [M]                 (activation per-token scales)
//   s_ch       fp32   [1, N]              (weight per-channel scales)
//   s_group    fp16   [K/128, N]          (weight per-group scales / s_ch; empty => per-channel)
//   workspace  int32  [>= N/64 * 16]      (locks, zero-initialised, reused)
//   c_tmp      int32  [16*64, N]          (int32 global-reduce scratch)
//   d          fp16   [M, N]              (output)
void marlin_qqq_gemm(
    tvm::ffi::TensorView a,
    tvm::ffi::TensorView b_q_weight,
    tvm::ffi::TensorView s_tok,
    tvm::ffi::TensorView s_ch,
    tvm::ffi::TensorView s_group,
  ...
  TensorMatcher({M, K}).with_device(device).verify(a);
  RuntimeCheck(M.unwrap() == size_m, "a.size(0) != size_m");
  RuntimeCheck(K.unwrap() == size_k, "a.size(1) != size_k");
  RuntimeCheck(size_k % 16 == 0, "size_k must be a multiple of 16");
  RuntimeCheck(b_q_weight.size(0) == size_k / 16, "b_q_weight.size(0) != size_k / 16");
  RuntimeCheck((b_q_weight.size(1) / 16) * 8 == size_n, "b_q_weight.size(1) does not match size_n");
  RuntimeCheck(s_tok.size(0) == size_m, "s_tok.size(0) != size_m");
  RuntimeCheck(size_n % marlin_qqq::min_thread_n == 0, "size_n must be a multiple of 64");
  RuntimeCheck(workspace.size(0) >= (size_n / marlin_qqq::min_thread_n) * marlin_qqq::max_par, "workspace too small");
  RuntimeCheck(c_tmp.size(0) >= marlin_qqq::max_par * 64 && c_tmp.size(1) == size_n, "c_tmp must be [max_par*64, N]");
  RuntimeCheck(d.size(0) == size_m && d.size(1) == size_n, "d must be [M, N]");
  int groupsize = (s_group.size(0) == 0) ? -1 : static_cast<int>(size_k / s_group.size(0));
  RuntimeCheck(groupsize == -1 || groupsize == 128, "groupsize must be 128 or per-channel");

  if (size_m == 0) return;
  DLDevice dl_device = device.unwrap();
  int dev = dl_device.device_id;
  cudaStream_t stream = LaunchKernel::resolve_device(dl_device);
  marlin_qqq::marlin_qqq_cuda(
      a.data_ptr(), b_q_weight.data_ptr(), c_tmp.data_ptr(), d.data_ptr(),
      s_tok.data_ptr(), s_ch.data_ptr(), s_group.data_ptr(),
      static_cast<int>(size_m), static_cast<int>(size_n), static_cast<int>(size_k),
      workspace.data_ptr(), groupsize, dev, stream, -1, -1, -1, marlin_qqq::max_par);
}
